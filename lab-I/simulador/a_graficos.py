...
class GestorGraficos:
    ...
    def actualizar_graficos(self, configuraciones, es_animacion=False):
        ...
        for i, (ax, config) in enumerate(zip(axes, configuraciones)):
            ...
            if config.get("show_periodo"):
                ax.axvline(x=periodo * 1000, color='magenta', linestyle='--', linewidth=1.5, label=f"Período ({periodo*1000:.1f} ms)")
                hay_graficos = True
            ...
            if i == n - 1: ax.set_xlabel("Tiempo (ms)")
            if hay_graficos: ax.legend(loc="lower left", fontsize=8)
